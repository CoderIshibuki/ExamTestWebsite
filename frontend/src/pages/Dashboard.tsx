...
import { Container, Typography, Box, Button, Card, CardContent, Grid, Avatar, AppBar, Toolbar, Chip } from '@mui/material';
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      <AppBar position="static" elevation={0} sx={{ bgcolor: 'white', borderBottom: '1px solid #E2E8F0' }}>
        <Toolbar>
          ...
          <Button variant="outlined" color="inherit" sx={{ color: 'text.secondary', borderColor: '#E2E8F0' }} onClick={logout}>
            Đăng xuất
          </Button>
        </Toolbar>
      </AppBar>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Card sx={{ height: '100%', borderRadius: 4, transition: 'all 0.3s ease', '&:hover': { boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)' } }}>
              <CardContent sx={{ p: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                <Avatar sx={{ width: 80, height: 80, bgcolor: 'primary.light', color: 'primary.dark', mb: 2, fontSize: 32 }}>
                ...
                <Chip icon={<VerifiedUser />} label={user?.role?.toUpperCase()} color="primary" size="small" sx={{ fontWeight: 600, px: 1 }} />
              </CardContent>
            </Card>
          </Grid>
            ...
            <Grid container spacing={3}>
                ...
                <Card sx={{ height: '100%', borderRadius: 4, transition: 'all 0.3s ease', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' } }}>
                  <CardContent sx={{ p: 4 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                      <Avatar sx={{ bgcolor: '#EEF2FF', color: '#4F46E5', mr: 2 }}><School /></Avatar>
                    ...
                    <Button variant="contained" fullWidth size="large" startIcon={<PlayArrow />} onClick={() => navigate('/exams')}>
                      Xem kỳ thi
                    </Button>
                  </CardContent>
                </Card>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <Card sx={{ height: '100%', borderRadius: 4, transition: 'all 0.3s ease', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' } }}>
                  <CardContent sx={{ p: 4 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                      <Avatar sx={{ bgcolor: '#ECFDF5', color: '#10B981', mr: 2 }}><FactCheck /></Avatar>
                ...
                <Card sx={{ borderRadius: 4, transition: 'all 0.3s ease', '&:hover': { boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' } }}>
                  <CardContent sx={{ p: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Avatar sx={{ bgcolor: '#FFFBEB', color: '#F59E0B', mr: 3 }}><Videocam /></Avatar>
                      <Box>
                    ...
                    <Button variant="outlined" color="warning" onClick={() => navigate('/camera-test')}>
                      Kiểm tra Camera
                    </Button>
                  </CardContent>
                </Card>
